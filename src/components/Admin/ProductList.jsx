...
const ProductList = ({ products, onEdit, onDelete }) => {
  return (
    <div className='my-8 w-full'>
      <hr />
      <h2  className='font-bold w-full text-center text-3xl my-4'>Product List</h2>
          ...
          {products.map((product) => (
            <tr key={product.id}>
              ...
              <td className='flex flex-row gap-4 mx-4'>
                ...
                <button onClick={() => onDelete(product.id)} type="submit" className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 m-2 px-4 rounded'>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
